    ...
    return (
        <div ref={rootRef} className="relative flex justify-end">
            <button
                ...
                aria-label={`Actions for ${bankAccount.name}`}
                aria-haspopup="menu"
                aria-expanded={open}
...
export default function BankAccountsIndex({ bankAccounts, filters, statuses }) {
    ...
    function openCreateModal() {
        setOpenMenuId(null);
        setModal({ open: true, mode: 'create', bankAccount: null });
    }

    function openEditModal(bankAccount) {
        setOpenMenuId(null);
        setModal({ open: true, mode: 'edit', bankAccount });
    }

    function closeModal() {
        setModal({ open: false, mode: 'create', bankAccount: null });
    }
    ...
    function deleteBankAccount(bankAccount) {
        if (!window.confirm(`Delete “${bankAccount.name}”?`)) {
            return;
        }

        router.delete(destroy.url(bankAccount.id));
    }

    function restoreBankAccount(bankAccount) {
        router.post(restore.url(bankAccount.id));
    }
            ...
            <div className="flex items-start justify-between gap-4 p-4">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-sm text-muted">
                        Bank accounts used when issuing post-dated checks for
                        purchase order pre-payments.
                    </p>
                    <p className="mt-1 text-sm font-medium text-muted">
                        Total: {bankAccounts.total}{' '}
                        {bankAccounts.total === 1
                            ? 'bank account'
                            : 'bank accounts'}
                    </p>
                </div>
                                ...
                                >
                                    ...
                                    <td className="py-3 pl-2">
                                        <RowActionsMenu
                                            ...
                                            open={
                                                openMenuId === bankAccount.id
                                            }
                                            onToggle={() =>
                                                setOpenMenuId((current) =>
                                                    current === bankAccount.id
                                                        ? null
                                                        : bankAccount.id,
                                                )
                                            }
