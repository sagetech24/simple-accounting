...
export default function AccountsReceivableShow({
    customer,
    order,
    paymentMethods = [],
    bankAccounts = [],
    ...
    const canRecordPayment = Boolean(order.can_add_payment);
    ...
    function voidPayment(payment) {
        ...
        router.delete(
            ...
            {
                data: { return_to: 'accounts-receivable' },
                preserveScroll: true,
            },
        );
    }
    ...
    return (
        <AppLayout title={`AR — ${order.reference}`}>
            <div className="flex flex-col gap-4 p-4">
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <span
                        className={`inline-flex rounded-full border px-3 py-1 text-xs ${paymentBadgeClass[status] ?? paymentBadgeClass.unpaid}`}
                    >
                        {paymentBadgeLabel[status] ?? 'Unpaid'}
                    </span>
                </div>
